...
const Streetwise = () => {
 ...
 useEffect(() => {
   ...
   map.current.on("load", () => {
     ...
     map.current.addSource("security-pins", {
       type: "geojson",
       data: {
         type: "FeatureCollection",
         features: [],
     ...
     map.current.addSource("escort-requests", {
       type: "geojson",
       data: {
         type: "FeatureCollection",
         features: [],
     ...
     map.current.addLayer({
       id: "security-heat",
       type: "heatmap",
       source: "security-pins",
       paint: {
         "heatmap-weight": ["get", "decay_weight"],
     ...
     // Add escort markers layer
     map.current.addLayer({
       id: "escort-markers",
       type: "circle",
       source: "escort-requests",
       paint: {
         "circle-radius": 8,
         "circle-color": "#00ff00",
     ...
     // Add click handler for security points
     map.current.on("click", "security-points", (e) => {
       const features = map.current.queryRenderedFeatures(e.point, {
         layers: ["security-points"],
       });
         ...
         const popup = new mapboxgl.Popup()
           ...
           .setHTML(
             ...
             <div class="p-3 max-w-xs">
               ...
               <p class="text-xs text-gray-500">Reported: ${new Date(
                 properties.created_at,
               ).toLocaleString()}</p>
               <p class="text-xs text-gray-500">Intensity: ${
     ...
     // Add click handler for escort markers
     map.current.on("click", "escort-markers", (e) => {
       const features = map.current.queryRenderedFeatures(e.point, {
         layers: ["escort-markers"],
       });
       if (features.length > 0) {
         const feature = features[0];
         const properties = feature.properties;
         ...
         const popup = new mapboxgl.Popup()
           ...
           .setHTML(
             `
             <div class="p-3 max-w-xs">
               <h3 class="font-bold text-lg mb-2">Buddy Up Request</h3>
               <p class="text-sm text-gray-700 mb-2">${properties.message}</p>
               <p class="text-xs text-gray-500">Requested: ${new Date(
                 properties.created_at,
               ).toLocaleString()}</p>
             </div>
           `,
     ...
     map.current.on("mouseenter", "security-points", () => {
       map.current.getCanvas().style.cursor = "pointer";
     });
     map.current.on("mouseleave", "security-points", () => {
       map.current.getCanvas().style.cursor = "";
     });


     map.current.on("mouseenter", "escort-markers", () => {
       map.current.getCanvas().style.cursor = "pointer";
     });
     map.current.on("mouseleave", "escort-markers", () => {
       map.current.getCanvas().style.cursor = "";
     });


     // Add source for road segments (lines between start and end points)
     map.current.addSource("road-segments", {
       type: "geojson",
       data: {
         type: "FeatureCollection",
         features: [],
     ...
     // Add layer for road segment lines
     map.current.addLayer({
       id: "road-lines",
       type: "line",
